...
document.addEventListener("DOMContentLoaded", function () {
    document
        .querySelectorAll(
            ".card, .stat-card, .chart-card, .table-card, .sales-table-card, .summary-card",
        )
...
function openInvoiceModal(invoiceNumber) {
    ...
    document.getElementById("invoiceTax").textContent = formatCurrency(
        invoice.tax,
    );
    ...
    document.getElementById("invoiceItemsBody").innerHTML = invoice.items
        .map(
            (item) => `
        <tr>
            <td>${item.name}</td>
        ...
        if (row) {
            ...
            if (tds.length >= 8) {
                ...
                if (isStockPage) {
                    ...
                    const contentDiv = popup.querySelector(
...
function closeProductDetails() {
    document.getElementById("productDetailsPopup").classList.remove("show");
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (themeBtn) {
        themeBtn.addEventListener("click", function () {
            ...
            // Temporarily enable transitions for smooth animation
    ...
    function updateClock() {
        ...
        if (!topbarDate || !topbarTime) return;

        const now = new Date();

        // Format date, e.g., "Mon, Jun 22, 2026"
        const dateOptions = {
            weekday: "short",
            month: "short",
            day: "numeric",
        ...
        topbarDate.textContent = now.toLocaleDateString("en-US", dateOptions);

        // Format time, e.g., "07:07:14 AM"
        const timeOptions = {
            hour: "2-digit",
            minute: "2-digit",
            second: "2-digit",
        ...
        topbarTime.textContent = now.toLocaleTimeString("en-US", timeOptions);
    }
